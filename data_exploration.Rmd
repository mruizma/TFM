...
plot_everything_tiff('test.tiff')
```
...
plot_everything_jpeg('test.jpg')
```

```{r}
# Pendent fer-ho millor
expr$`# gens expressats` <- as.numeric(expr$`# gens expressats`)
jpeg(filename = 'expressio.jpg',
     res = 300,
     width = 10, height = 8, units = 'in')
par(mar = c(7, 4, 2, 2) + 0.4)
barplot(expr$`# gens expressats`, main='Nombre de gens expressats', names.arg=expr$Regió, cex.names=0.65, las=2)
dev.off()
```
...
```{r}
# Provisional
sobreexp_final <- sobreexp
sobreexp_final$AD <- comparison[, 1]
```
...
library(corrplot)
rounded_hippo <- round(cormat_hippo, 3)
